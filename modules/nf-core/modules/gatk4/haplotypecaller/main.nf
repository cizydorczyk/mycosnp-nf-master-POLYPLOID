process GATK4_HAPLOTYPECALLER {
    ...
    input:
    tuple val(meta), path(input), path(input_index), path(intervals), val(task_ext_args)  // Added last val input
    path fasta
    path fai
    path dict
    path dbsnp
    ...
    script:
    // Use the passed-in per-task arguments or empty string if missing
    def args = task_ext_args ?: ''
