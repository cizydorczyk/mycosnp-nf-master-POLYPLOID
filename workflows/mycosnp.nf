...
params.ploidy_file = params.ploidy_file ?: null

def ploidy_map = [:]
if (params.ploidy_file) {
    ploidy_map = file(params.ploidy_file).readLines()
                  .collectEntries { line ->
                      def (sample, ploidy) = line.tokenize('\t')
                      [(sample): ploidy.toInteger()]
                  }
}
...
*/
    ...
    GATK4_HAPLOTYPECALLER( aligned_with_ploidy, fas_file, fai_file, dict_file, [], [] )
