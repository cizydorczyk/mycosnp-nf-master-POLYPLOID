...
workflow BWA_PREPROCESS {
    ...
    // FASTQC_POST(SEQKIT_PAIR.out.reads)
    }
    else {
    DOWNSAMPLE_RATE(reads, reference[0], params.coverage)
    ch_seq_samplerate = reads.join(
        DOWNSAMPLE_RATE.out.downsampled_rate.map{ meta, sr, snr -> [ meta, snr]}
        )
    SEQTK_SAMPLE(ch_seq_samplerate)
    BWA_MEM(SEQTK_SAMPLE.out.reads, reference[2], true)
    // FASTQC_POST(SEQTK_SAMPLE.out.reads)
    }
